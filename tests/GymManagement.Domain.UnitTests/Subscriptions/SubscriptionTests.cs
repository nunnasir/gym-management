...
{
    [Fact]
    public void AddGym_WhenMoreThanSubsciptionAllows_ShouldFail()
    {
        // Arrange
        var subscription = SubscriptionFactory.CreateSubscription();

        // Create the maximum number of gyms + 1
        var gyms = Enumerable.Range(0, subscription.GetMaxGym() + 1)
        ...
        var lastAddGymResult = addGymResults.LastOrDefault();
