...
public class CreateSubscriptionTests
{
    ...
    public async Task CreateSubscription_WhenValidSubscription_ShouldCreateSubscription(SubscriptionType subscriptionType)
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync("Subscription", createSubscriptionRequest);
        ...
        response.Headers.Location!.PathAndQuery.Should().Be($"/Subscription/{subscriptionResponse.Id}");
    }
